...
# 桌面版由 main.py 透過環境變數注入路徑；一般執行時 fallback 到原本位置
DB_PATH    = Path(os.environ.get("FSC_DB_PATH",    BASE_DIR / "fsc_ebank.db"))
UPLOAD_DIR = Path(os.environ.get("FSC_UPLOAD_DIR", BASE_DIR / "uploads"))
UPLOAD_DIR.mkdir(parents=True, exist_ok=True)

# 桌面版打包後 templates/static 在 BUNDLE_DIR 下
_bundle = os.environ.get("FSC_BUNDLE_DIR")
_tmpl   = str(Path(_bundle) / "templates") if _bundle else "templates"
_static = str(Path(_bundle) / "static")    if _bundle else "static"
